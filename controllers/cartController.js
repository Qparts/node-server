...
const getQuotation = (req, res) => {
	...
	apiGetRequest(
		`${GET_QUOTATION_CART_URL}/${customerId}`,
...
const getRepliedQuotation = (req, res) => {
	const { customerId } = req.params;
	apiGetRequest(
		`${GET_QUOTATION_CART_URL}/${customerId}/submitted`,
		req.session.customer
	).then(data => {
		if (data.statusCode !== 200) {
			res.sendStatus(500);
...
const addCart = (req, res) => {
	apiPostRequest(ADD_QUOTATION_CART_URL, req.body, req.session.customer).then(
		data => {
			if (data.statusCode !== 200) {
				res.sendStatus(500);
			} else {
...
const getBanks = (req, res) => {
	apiGetRequest(GET_BANKS_URL, req.session.customer).then(data => {
		if (data.statusCode === 200) {
			res.send(data.body);
		} else {
			res.sendStatus(data.statusCode);
