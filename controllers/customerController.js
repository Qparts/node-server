...
const editName = (req, res) => {
	...
	apiPutRequest(
		CHANGE_NAME_URL,
		{ firstName, lastName, defaultLang, customerId },
...
const resetPassword = (req, res) => {
	apiPostRequest(RESET_PASSWORD_URL, req.body).then(data => {
...
const resetPasswordToken = (req, res) => {
	...
	apiGetRequest(`${RESET_PASSWORD_URL}/token/${tokenId}`, req.body).then(
		data => {
			if (data.statusCode !== 201) {
				res.sendStatus(500);
			} else {
...
const updatePassword = (req, res) => {
	...
	apiPutRequest(RESET_PASSWORD_URL, { password, token }).then(data => {
		if (data.statusCode !== 200) {
			res.sendStatus(500);
		} else {
...
const resetPasswordSms = (req, res) => {
	apiPostRequest(RESET_SMS_URL, req.body).then(data => {
		if (data.statusCode === 401 || data.statusCode === 404) {
			res.status(data.statusCode).send(
				errorMessages.form.signin.resetPassword
			);
		} else {
...
const login = (req, res) => {
	apiPostRequest(LOGIN_URL, req.body).then(data => {
		if (data.statusCode === 200) {
			saveCurrentCustomer(req, data.body);
			res.send(data.body);
		} else {
			res.status(data.statusCode).send(
				errorMessages.form.signin.incorrectPassword
			);
		}
	});
};
...
const postCodeLogin = (req, res) => {
	apiPostRequest(CODE_LOGIN_URL, req.body).then(data => {
		if (data.statusCode === 200) {
			saveCurrentCustomer(req, data.body);
			res.send(data.body);
		} else {
...
const socialMediaAuth = (req, res) => {
	apiPostRequest(SOCIAL_MEDIA_AUTH_URL, { ...req.body, countryId: 1 }).then(
		data => {
			if (data.statusCode !== 200) {
				res.sendStatus(data.statusCode);
			} else {
...
const signup = (req, res) => {
	apiPostRequest(SIGNUP_URL, req.body.customer).then(data => {
		if (data.statusCode === 409) {
			res.status(data.statusCode).send({
				error: errorMessages.form.signup.email,
				field: 'email'
			});
...
const accountVerification = (req, res) => {
	apiPostRequest(ACCOUNT_VERIFY_URL, req.body).then(data => {
		if (data.statusCode === 200) {
			res.send(data.body);
		} else {
			res.sendStatus(data.statusCode);
...
const addSocialMediaLink = (req, res) => {
	...
	apiPostRequest(
		SOCIAL_MEDIA_LINK_URL,
		{ platform, socialMediaId, customerId, email },
		req.session.customer
	).then(data => {
		if (data.statusCode === 201) {
			res.sendStatus(data.statusCode);
			...
			res.status(data.statusCode).send({
				error: errorMessages.socialMediaLink.usedEmail
			});
		} else {
			res.sendStatus(500);
		}
...
const addVehicle = (req, res) => {
	...
	apiPostRequest(
		ADD_VEHICLE_URL,
		{ customerId, vehicleYearId, vin, defaultVehicle, imageAttached },
		req.session.customer
	).then(data => {
		if (data.statusCode !== 200) {
			res.sendStatus(500);
...
const changeEmailSendLink = (req, res) => {
	...
	apiPostRequest(
		...
		if (data.statusCode === 201) {
			...
			res.status(data.statusCode).send(
				errorMessages.form.changeEmail.email
			);
		} else {
			res.sendStatus(500);
		}
...
const changeEmailVerification = (req, res) => {
	...
	apiPutRequest(CHANGE_EMAIL_URL, { code, email }).then(data => {
		if (data.statusCode === 201) {
...
const changePassword = (req, res) => {
	...
	apiPutRequest(
		...
		if (data.statusCode === 201) {
			...
			res.status(data.statusCode).send(
				errorMessages.form.changePassword.password
			);
		}
	});
};
...
const matchCode = (req, res, next) => {
	...
	if (req.session.code !== req.body.code)
		res.status(403).send('verification code is not matched');
	else {
		next();
	}
};
...
const logout = (req, res) => {
	const customerId = getCustomerId(req);
	apiPostRequest(LOGOUT_URL, customerId, req.session.customer).then(data => {
		if (data.statusCode !== 200) {
			res.sendStatus(500);
		} else {
			req.session.destroy();
